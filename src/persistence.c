...
LOG_MODULE_REGISTER(anjay_zephyr_persistence);
    ...
    for (size_t i = 0; i < AVS_ARRAY_SIZE(targets); i++) {
        if (!strcmp(key, targets[i].name)) {
            ...
            if (read_cb(cb_arg, buf, len) >= 0
                    && avs_is_ok(targets[i].restore(
                               args->anjay, (avs_stream_t *) &stream))) {
                args->left_to_read--;
                result = 0;
...
static int restore_anjay_from_settings(anjay_t *anjay,
                                       const char *root_name,
                                       bool purge_on_fail) {
    assert(anjay);
    assert(root_name);

    struct persistence_load_args args = {
        .anjay = anjay,
        .left_to_read = AVS_ARRAY_SIZE(targets),
        .root_name = root_name
    };

    if (settings_load_subtree_direct(root_name, persistence_load,
                                     (void *) &args)
            || args.left_to_read > 0) {
...
int _anjay_zephyr_restore_anjay_from_factory_provisioning(anjay_t *anjay) {
    return restore_anjay_from_settings(anjay, FACTORY_PROVISIONING_ROOT_NAME,
                                       false);
}
#endif // CONFIG_ANJAY_ZEPHYR_FACTORY_PROVISIONING
...
static int provisioning_info_present_cb(const char *key,
                                        size_t len,
                                        settings_read_cb read_cb,
                                        void *cb_arg,
                                        void *param) {
...
bool anjay_zephyr_is_factory_provisioning_info_present(void) {
    ...
    settings_load_subtree_direct(FACTORY_PROVISIONING_ROOT_NAME,
                                 provisioning_info_present_cb,
