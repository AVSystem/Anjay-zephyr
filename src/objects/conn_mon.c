...
static int list_resources(anjay_t *anjay,
    ...
    anjay_dm_emit_res(ctx, RID_CONN_MON_NETWORK_BEARER, ANJAY_DM_RES_R,
                      ANJAY_DM_RES_PRESENT);
    anjay_dm_emit_res(ctx, RID_CONN_MON_AVAILABLE_NETWORK_BEARER,
                      ANJAY_DM_RES_RM, ANJAY_DM_RES_PRESENT);
    anjay_dm_emit_res(ctx, RID_CONN_MON_RSS, ANJAY_DM_RES_R,
                      ANJAY_DM_RES_PRESENT);
    anjay_dm_emit_res(ctx, RID_CONN_MON_LINK_QUALITY, ANJAY_DM_RES_R,
                      ANJAY_DM_RES_PRESENT);
    anjay_dm_emit_res(ctx, RID_CONN_MON_IP_ADDRESSES, ANJAY_DM_RES_RM,
                      ANJAY_DM_RES_PRESENT);
    anjay_dm_emit_res(ctx, RID_CONN_MON_CELL_ID, ANJAY_DM_RES_R,
                      ANJAY_DM_RES_PRESENT);
    anjay_dm_emit_res(ctx, RID_CONN_MON_SMNC, ANJAY_DM_RES_R,
                      ANJAY_DM_RES_PRESENT);
    anjay_dm_emit_res(ctx, RID_CONN_MON_SMCC, ANJAY_DM_RES_R,
                      ANJAY_DM_RES_PRESENT);
    anjay_dm_emit_res(ctx, RID_CONN_MON_LAC, ANJAY_DM_RES_R,
                      ANJAY_DM_RES_PRESENT);
    return 0;
}
...
const anjay_dm_object_def_t **_anjay_zephyr_conn_mon_object_create(
    ...
    struct conn_mon_object *obj = (struct conn_mon_object *) avs_calloc(
            1, sizeof(struct conn_mon_object));
    if (!obj) {
        return NULL;
    }
    ...
    if (cell_info->id != LTE_LC_CELL_EUTRAN_ID_INVALID) {
        ...
        AVS_STATIC_ASSERT(sizeof(obj->ip_address)
                                  == sizeof(nrf_lc_info->ip_addr),
