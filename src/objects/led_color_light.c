...
static inline struct led_color_light_object *
...
static int rgb_value_str_parse(struct led_color_light_object *obj) {
    ...
    if (avs_unhexlify(&out_bytes_written, obj->rgb_value,
                      sizeof(obj->rgb_value), rgb_value_hexstring,
                      rgb_value_hexstring_strlen)
...
#define SET_COLOR(pin) \
    pwm_set_cycles(obj->dev, pin, 255, obj->rgb_value[pin], 0)
...
static int list_resources(anjay_t *anjay,
    ...
    anjay_dm_emit_res(ctx, RID_RGB_VALUE, ANJAY_DM_RES_RW,
...
static int resource_write(anjay_t *anjay,
    ...
    case RID_RGB_VALUE: {
        ...
        int err =
                anjay_get_string(ctx, obj->rgb_value_str, RGB_VALUE_STR_BUFLEN);
...
anjay_zephyr_led_color_light_object_create(const struct device *user_device) {
    if (!device_is_ready(user_device)) {
        return NULL;
    }

    struct led_color_light_object *obj =
            (struct led_color_light_object *) avs_calloc(
    ...
    if (rgb_led_set(obj)) {
        ...
        anjay_zephyr_led_color_light_object_release(&out_def);
        return NULL;
    }
