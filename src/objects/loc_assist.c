...
LOG_MODULE_REGISTER(anjay_zephyr_loc_assist);
...
static inline struct loc_assist_object *
...
static int list_resources(anjay_t *anjay,
    ...
    anjay_dm_emit_res(ctx, RID_ASSISTANCE_TYPE, ANJAY_DM_RES_R,
                      ANJAY_DM_RES_PRESENT);

#ifdef CONFIG_ANJAY_ZEPHYR_GPS_NRF_A_GPS
    anjay_dm_emit_res(ctx, RID_A_GPS_ASSISTANCE_MASK, ANJAY_DM_RES_R,
                      ANJAY_DM_RES_PRESENT);
#endif // CONFIG_ANJAY_ZEPHYR_GPS_NRF_A_GPS
...
#    error "P-GPS not implemented yet"
    ...
    anjay_dm_emit_res(ctx, RID_ASSISTANCE_DATA, ANJAY_DM_RES_W,
                      ANJAY_DM_RES_PRESENT);
#endif // CONFIG_ANJAY_ZEPHYR_LOCATION_SERIVCES_ASSISTANCE

    anjay_dm_emit_res(ctx, RID_RESULT_CODE, ANJAY_DM_RES_W,
                      ANJAY_DM_RES_PRESENT);
...
static int transaction_begin(anjay_t *anjay,
    ...
    memcpy(&obj->location_backup, &obj->location_current,
...
static int transaction_commit(anjay_t *anjay,
    ...
    if (obj->location_current.latitude != obj->location_backup.latitude
            ...
            || obj->location_current.accuracy
                           != obj->location_backup.accuracy) {
...
#ifdef CONFIG_ANJAY_ZEPHYR_LOCATION_SERVICES_MANUAL_CELL_BASED
    memcpy(&obj->location_current, &obj->location_backup,
           sizeof(obj->location_backup));
#endif // CONFIG_ANJAY_ZEPHYR_LOCATION_SERVICES_MANUAL_CELL_BASED
...
const anjay_dm_object_def_t **_anjay_zephyr_loc_assist_object_create(void) {
    struct loc_assist_object *obj = (struct loc_assist_object *) avs_calloc(
            1, sizeof(struct loc_assist_object));
...
static int batch_compile_and_send(anjay_t *anjay,
    ...
    // best effort - we're sending the request to all servers, although some of
    // them might not answer with correct location at all
    ...
    AVS_LIST_FOREACH(ssid, ssids) {
        anjay_send_result_t send_result =
        ...
        if (is_deferrable_condition(send_result)) {
            LOG_WRN("Target SSID=%" PRIu16 " is offline, attempting "
                    ...
                    *ssid);
            send_result =
                    anjay_send_deferrable(anjay, *ssid, batch, NULL, NULL);
        }
...
static int add_conn_mon_to_batch(anjay_t *anjay,
    ...
    int result = anjay_send_batch_data_add_current_multiple(
            builder, anjay, conn_mon_paths, AVS_ARRAY_SIZE(conn_mon_paths));
    if (result) {
...
void _anjay_zephyr_loc_assist_object_send_agps_request(
    ...
    static const struct {
        ...
        {
            .req_flag = LOC_ASSIST_A_GPS_MASK_UTC,
            .name = "UTC parameters"
        },
        {
        ...
        {
            .req_flag = LOC_ASSIST_A_GPS_MASK_TOW,
            .name = "SV time of week"
        },
        {
            .req_flag = LOC_ASSIST_A_GPS_MASK_CLOCK,
        ...
        {
            .req_flag = LOC_ASSIST_A_GPS_MASK_EPHEMERIS,
            .name = "GPS ephemeris"
        },
        {
            .req_flag = LOC_ASSIST_A_GPS_MASK_ALMANAC,
            .name = "GPS almanac"
        }
    };
    ...
    int result = anjay_send_batch_add_int(builder, OBJ_DEF.oid, 0,
                                          RID_ASSISTANCE_TYPE, UINT16_MAX,
                                          avs_time_real_now(), 5);
    if (result) {
    ...
    // appropriate
    result = anjay_send_batch_add_int(builder, OBJ_DEF.oid, 0,
                                      RID_A_GPS_ASSISTANCE_MASK, UINT16_MAX,
                                      avs_time_real_now(), request_mask);
    if (result) {
        LOG_ERR("Failed to add assistance type to batch, err: %d", result);
        goto finalize_batch;
    }
...
void _anjay_zephyr_loc_assist_object_send_cell_request(
    ...
    int result = anjay_send_batch_add_int(builder, OBJ_DEF.oid, 0,
                                          RID_ASSISTANCE_TYPE, UINT16_MAX,
                                          avs_time_real_now(), request_type);
    if (result) {
        LOG_ERR("Failed to add assistance type to batch, err: %d", result);
        goto finalize_batch;
    }
    ...
    if (request_type == LOC_ASSIST_CELL_REQUEST_INFORM_MULTI
        ...
        uint8_t ecid_instance_count =
                _anjay_zephyr_ecid_object_instance_count(ecid_def);
