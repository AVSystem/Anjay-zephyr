...
struct buzzer_object {
    ...
    // anjay_zephyr_buzzer_object_update call
    // the concurring threads are:
    // - update_objects thread calling anjay_zephyr_buzzer_object_update
...
static inline struct buzzer_object *
...
static int buzzer_enable(struct buzzer_object *obj) {
    ...
    return pwm_set(obj->user_device->device, obj->user_device->pin,
                   PWM_USEC(period), PWM_USEC(pulse), 0);
}

static int buzzer_disable(struct buzzer_object *obj) {
    return pwm_set(obj->user_device->device, obj->user_device->pin, 0, 0, 0);
}
...
static int buzzer_reschedule(struct buzzer_object *obj) {
    bool was_scheduled =
            k_work_cancel_delayable_sync(&obj->disable_buzzer_dwork,
                                         &obj->sync);
...
static int list_resources(anjay_t *anjay,
    ...
    anjay_dm_emit_res(ctx, RID_DELAY_DURATION, ANJAY_DM_RES_RW,
                      ANJAY_DM_RES_PRESENT);
    anjay_dm_emit_res(ctx, RID_MINIMUM_OFF_TIME, ANJAY_DM_RES_RW,
                      ANJAY_DM_RES_PRESENT);
...
static int resource_write(anjay_t *anjay,
    ...
    case RID_APPLICATION_TYPE: {
        ...
        int err = anjay_get_string(ctx, obj->state.application_type,
                                   APPLICATION_TYPE_BUFLEN);
...
static int transaction_validate(anjay_t *anjay,
    ...
    if (obj->state.on_off && obj->last_run_end_timestamp != TIMESTAMP_NONE) {
        int64_t minimum_off_time_msecs =
                MSEC_PER_SEC * obj->state.minimum_off_time;
        int64_t next_run_timestamp =
                obj->last_run_end_timestamp + minimum_off_time_msecs;
...
const anjay_dm_object_def_t **anjay_zephyr_buzzer_object_create(
        const struct anjay_zephyr_buzzer_device *user_device) {
    if (!device_is_ready(user_device->device)) {
        return NULL;
    }

    struct buzzer_object *obj =
            (struct buzzer_object *) avs_calloc(1,
                                                sizeof(struct buzzer_object));
    if (!obj) {
        return NULL;
    }
    obj->def = &obj_def;
    ...
    k_work_init_delayable(&obj->disable_buzzer_dwork,
                          buzzer_disable_work_handler);
    ...
    if (buzzer_reschedule(obj)) {
        ...
        anjay_zephyr_buzzer_object_release(&out_def);
        return NULL;
    }
...
void anjay_zephyr_buzzer_object_update(
...
void anjay_zephyr_buzzer_object_release(
