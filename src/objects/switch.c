...
static inline struct switch_object *
get_obj(const anjay_dm_object_def_t *const *obj_ptr) {
    assert(obj_ptr);
    return AVS_CONTAINER_OF(obj_ptr, struct switch_object, def);
}

static struct switch_instance *find_instance(const struct switch_object *obj,
                                             anjay_iid_t iid) {
...
static struct switch_instance *add_instance(struct switch_object *obj,
                                            anjay_iid_t iid) {
    assert(find_instance(obj, iid) == NULL);

    AVS_LIST(struct switch_instance) created =
            AVS_LIST_NEW_ELEMENT(struct switch_instance);
    if (!created) {
        return NULL;
    }
...
static int configure_switch(struct switch_object *obj,
                            const struct device *dev,
                            int gpio_pin,
                            int gpio_flags,
                            anjay_iid_t iid) {
...
void anjay_zephyr_switch_object_release(
...
const anjay_dm_object_def_t **anjay_zephyr_switch_object_create(
        struct anjay_zephyr_switch_instance *user_swtiches,
        size_t user_switches_array_length) {
    struct switch_object *obj =
            (struct switch_object *) avs_calloc(1,
                                                sizeof(struct switch_object));
    if (!obj) {
        return NULL;
    }
    obj->def = obj_def_ptr;

    for (anjay_iid_t iid = 0; iid < user_switches_array_length; iid++) {
        configure_switch(obj, user_swtiches[iid].device,
                         user_swtiches[iid].gpio_pin,
                         user_swtiches[iid].gpio_flags, iid);
    }

    if (!obj->instances) {
        const anjay_dm_object_def_t **out_def = &obj_def_ptr;

        anjay_zephyr_switch_object_release(&out_def);
        return NULL;
    }
...
void anjay_zephyr_switch_object_update(
