...
#ifdef CONFIG_ANJAY_ZEPHYR_LOCATION_OBJECT
struct location_object {
    const anjay_dm_object_def_t *def;
    struct anjay_zephyr_gps_data cached_read;
};

static inline struct location_object *
...
#    if CONFIG_ANJAY_ZEPHYR_GPS_SPEED
...
const anjay_dm_object_def_t **anjay_zephyr_location_object_create(void) {
    struct location_object *obj = (struct location_object *) avs_calloc(
            1, sizeof(struct location_object));
    if (!obj) {
        return NULL;
    }
    obj->def = &OBJ_DEF;
    ...
    SYNCHRONIZED(anjay_zephyr_gps_read_last_mtx) {
        if (anjay_zephyr_gps_read_last.valid
                && anjay_zephyr_gps_read_last.timestamp
                               != obj->cached_read.timestamp) {
            obj->cached_read = anjay_zephyr_gps_read_last;
        }
    }
...
#else  // CONFIG_ANJAY_ZEPHYR_LOCATION_OBJECT
const anjay_dm_object_def_t **anjay_zephyr_location_object_create(void) {
    return NULL;
}
...
#endif // CONFIG_ANJAY_ZEPHYR_LOCATION_OBJECT
