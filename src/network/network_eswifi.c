...
static void eswifi_reconnect_work_cb(struct k_work *work) {
    net_mgmt(NET_REQUEST_WIFI_CONNECT, eswifi_by_iface_idx(0)->iface,
...
static void eswifi_keepalive_work_cb(struct k_work *work) {
    ...
    bool connected = (eswifi_at_cmd_rsp(eswifi, "CS\r", &result) >= 0 && result
                      && result[0] == '1');
    ...
    if (!connected) {
        ...
        // Issuing NET_REQUEST_WIFI_CONNECT locks the eswifi mutex for 30
...
static void eswifi_disconnect_work_cb(struct k_work *work) {
    ...
    while (true) {
        struct in_addr *addr =
                net_if_ipv4_get_ll(eswifi->iface, NET_ADDR_ANY_STATE);
...
static void eswifi_mgmt_cb(struct net_mgmt_event_callback *cb,
                           uint32_t mgmt_event,
                           struct net_if *iface) {
...
int _anjay_zephyr_network_connect_async(void) {
    ...
    int ret = net_mgmt(NET_REQUEST_WIFI_CONNECT, eswifi_by_iface_idx(0)->iface,
                       &wifi_params, sizeof(struct wifi_connect_req_params));
...
void _anjay_zephyr_network_disconnect(void) {
    ...
    net_mgmt(NET_REQUEST_WIFI_DISCONNECT, eswifi_by_iface_idx(0)->iface, NULL,
             0);
}
